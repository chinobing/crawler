...
class JuShuoSheSearch(object):
    """
    利用搜狗搜索得到的微信文章的结果.
    """

    search_url = "http://voice.itjuzi.com/?s={}"
    search_page_url = "http://voice.itjuzi.com/?s={}&paged={}"
    ...
    def search(key_word, search_page=5):
        ...
        try:
            ...
            url = JuShuoSheSearch.search_url.format(key_word_quote)
            failure = 0
            # url用来控是否有下一页, failure 用来控制失败次数
            while len(url) > 0 and failure < 10:
                # 有简单的防爬虫机制的话就加上headers
                ...
                if r.status_code == 200:
                    ...
                    result_links = bs_obj.find(id="main").findAll("article")
                    for result in result_links:
                        try:
                            item_ins = deepcopy(JuShuoSheSearch.item)
                            title_node = result.find("h1",class_="entry-title")
                            item_ins['link'] = title_node.find("a").get("href")
                            item_ins['title'] = title_node.find("a").get_text()
                            item_ins['abstract'] = result.find("div", class_="entry-summary").get_text()
                            item_ins["date"] = result.find("time").get_text()
                            # item_ins["date"]=item_ins["date"][-11:]
                            result_list["result_list"].append(item_ins)
                        except BaseException as e:
                            logging.error("Parse JuShuoShe result error. ErrorMsg: %s" % str(e))
                    next_page = bs_obj.find("div", class_="nav-previous button").find("a")
                    if next_page:
            ...
            logging.error("Get JuShuoShe search result error. ErrorMsg: %s" % str(e))
        return result_list
    ...
    def search_page(key_word, page):
        ...
        try:
            key_word_quote = parse.quote(key_word)
            url = JuShuoSheSearch.search_page_url.format(key_word_quote,page)
            failure = 0
            # url用来控是否有下一页, failure 用来控制失败次数
            while len(url) > 0 and failure < 10:
                # 有简单的防爬虫机制的话就加上headers
...
if __name__ == "__main__":
    url_list = JuShuoSheSearch.search_page("人工智能", page=3)  # 已经测试过了,运行正常
