...
class SoGouSearch(object):
    ...
    page_url_format = "https://www.sogou.com/web?query={}"  # 匹配parse过的关键词即可.

    # 360搜索引擎最新消息(新闻页面的搜索)的url
    news_url_format = "http://news.sogou.com/news?mode=1&sort=0&fixrank=1&query={}&shid=gd}"

    sogou_url = "https://www.sogou.com/web"   # 此处需要加上web,因为下一页给出的地址只有参数
    ...
    def parse_link(link):
        """
        parse the link of sogou search to target link URL.
        :param link: origin link from sogou search engine
        :return: target link
        """
        r = requests.get(link, allow_redirects=False)
        content = r.content.decode("utf-8")
        pattern = re.compile(r"URL=\'(.*?)\'")
        url = pattern.findall(content)
        if url:
            return url[0]
        else:
            return ""
    ...
    def search(key_word, search_page=5):
        ...
        try:
            ...
            url = SoGouSearch.page_url_format.format(key_word_quote)
            failure = 0
            ...
            while len(url) > 0 and failure < 10:
                ...
                if r.status_code == 200:
                    ...
                    for result in result_links:
                        try:
                            ...
                            if not contains and link:
                                title = link.get_text()
                                href = SoGouSearch.parse_link(link.find("a").get("href"))
                                if len(href) == 0:
                                    continue
                                ...
                                if title and url and abstract:
                                    item_ins = deepcopy(SoGouSearch.item)
                                    item_ins['link'] = href
                    ...
                    if next_page:
                        url = SoGouSearch.sogou_url + next_page.get("href")
                    else:
                        url = ""
                    failure = 0
                    max_page -= 1
...
if __name__ == "__main__":
    url_list = SoGouSearch.search_page("区块链", 1)    # 已经测试过了,运行正常
